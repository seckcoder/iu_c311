...
(define interp-exp 
  (lambda (exp env)
    (cases
      ...
      (newpair-exp
        (left-exp right-exp)
        (make-pair (interp-exp left-exp env)
                   (interp-exp right-exp env)))
      (pair-left-exp
        (pair-exp)
        (pleft (interp-exp pair-exp env)))
      (pair-right-exp
        (pair-exp)
        (pright (interp-exp pair-exp env)))
      (set-pair-left-exp
        (pair-exp left-value-exp)
        (setleft (interp-exp pair-exp env)
                 (interp-exp left-value-exp env)))
      (set-pair-right-exp
        (pair-exp right-value-exp)
        (setright (interp-exp pair-exp env)
                  (interp-exp right-value-exp env)))
      )))
...
(define (test)
  ...
  ; test pair
  (test-prog-eqv "let glo = pair(11,22)
                  in let f = proc(loc)
                     ...
                     in (f glo)"
                  88)
  (test-prog-eqv "let f = proc(apair)
                           setleft(apair, 11)
                  in let p = pair(2,3)
                    in {
                      (f p);
