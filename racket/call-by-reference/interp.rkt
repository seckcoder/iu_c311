...
(define interp-rand-exp
  (lambda (exp env)
    (cases expression exp
      (var-exp
        (var)
        (apply-env env var)) ; directly return reference
      (else
        (newref (interp-exp exp env))))))
    ...
    (cases
      ...
      (call-exp
        ...
        (let ((proc (interp-exp proc-exp env))
              (rand-refs (map (lambda (rand-exp)
                           (interp-rand-exp rand-exp env))
                         rand-exps)))
          (apply-proc proc rand-refs)))
      (letrec-exp
...
(define (test)
  ...
  (test-prog-eqv "let b = 3
                  in let p = proc(x)
                      ...
                      in ((p b) b)"
                  4) ; a really tricky program(variable aliasing)
