              ...
              (if (= except-id searched-id)
                (begin
                  (let ((new-env (extend-env cont-var
                                             (newref (procval (contproc saved-cont)))
                                             env)))
...
(define apply-cont
  (lambda (cont exp-val)
    (cases continuation cont
      ...
      (mult1-cont
        (exp2 env next-cont)
        (interp-exp/k exp2 env (mult2-cont exp-val next-cont)))
      (mult2-cont
        (val1 next-cont)
