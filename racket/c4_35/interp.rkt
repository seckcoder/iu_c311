...
(define interp-exp 
  (lambda (exp env)
    (cases
      ...
      (deref-exp
        (var-exp)
        ; here we have var = some-thing
        ; some-thing is mapped to an address in store
        ; and that address that will be derefed is then mapped to the real value
        (deref (interp-exp var-exp env)))
      (setref-exp
        (var-exp val-exp)
        ; Notice that setref is different from set
        ; what set receive is identifier while
        ; setref will receive expression!
        (setref! (interp-exp var-exp env)
                 (interp-exp val-exp env)))
      (println-exp
        (exp)
        (println "~s" (interp-exp exp env)))
      )))
